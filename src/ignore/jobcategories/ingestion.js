/**
 * jobcategories/ingestion.js
 * Initialize the `jobcategories` collection in MongoDB.
...
const initializeDatabase = async (collectionName, fileName) => {
  try {
    ...
    for(var i=0; i < rawData.length; i++){
        const includedCoverages = rawData[i].includedCoverages.split('|');
        const excludedCoverages = rawData[i].excludedCoverages.split('|');
        rawData[i].includedCoverages = includedCoverages;
        rawData[i].excludedCoverages = excludedCoverages;
    }
...
initializeDatabase('jobcategories', 'jobcategories.csv').catch((err) => {
  console.error(err);
});
